...
const (
	...
	adRotateRoleEndpoint = "ad/rotate-role/"
)
...
func (sdk vaultSDK) ADRotateRoleCreds(name string) (string, error) {
	url := fmt.Sprintf("%s/%s/%s", sdk.baseURL, adRotateRoleEndpoint, name)

	_, _, err := sdk.processRequest(http.MethodPost, url, nil, http.StatusNoContent)
	if err != nil {
		return "", err
	}
